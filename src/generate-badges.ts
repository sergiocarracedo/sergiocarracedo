...
export function generateBadgeTable(stats: GitHubStats): string {
  ...
  // Generate badges only (no table since badges already include labels)
  const badges = `
![Stars](${createBadge('Stars', formatNumber(totalStars), '4c8eda')})
![Repos](${createBadge('Repositories', formatNumber(totalRepos), '2ea44f')})
![Forks](${createBadge('Forks', formatNumber(totalForks), 'f39c12')})
![Followers](${createBadge('Followers', formatNumber(followers), '8e44ad')})
![Languages](${createBadge('Languages', languagesStr, 'e74c3c')})
`.trim();
